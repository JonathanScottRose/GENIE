...
{
  REAL *duals0=0, *dualsfrom0=0, *dualstill0=0;
...
{
  REAL *objfrom0=0, *objtill0=0, *objfromvalue0=0, *objtillvalue0=0;
  ...
  /* Basis adjustments due to deletions (after actual row/column deletions) */
    ...
    if(!isrow && (k < lp->rows)) {
      ...
      for(i = 1; (i <= lp->rows) && (k < lp->rows); i++)
        if(!lp->is_basic[i]) {
          if(!is_constr_type(lp, i, LPS_EQ) || (j == 1)) {
            k++;
            lp->var_basic[k] = i;
            lp->is_basic[i] = TRUE;
          }
...
MYBOOL __WINAPI add_constraintex(lprec *lp, int count, REAL *row, int *colno, int constr_type, REAL rh)
{
  ...
  if(!(constr_type == LPS_LE || constr_type == LPS_GE || constr_type == LPS_EQ)) {
  ...
  if((constr_type & ROWTYPE_CONSTRAINT) == LPS_EQ) {
    lp->equalities++;
...
MYBOOL __WINAPI del_constraint(lprec *lp, int rownr)
{
  ...
  */
  ...
  if(is_constr_type(lp, rownr, LPS_EQ) && (lp->equalities > 0))
    lp->equalities--;
...
MYBOOL __WINAPI add_lag_con(lprec *lp, REAL *row, int con_type, REAL rhs)
{
  ...
  if(con_type == LPS_LE || con_type == LPS_EQ)
    sign = 1;
  else if(con_type == LPS_GE)
    sign = -1;
  else {
...
MYBOOL __WINAPI set_rh_range(lprec *lp, int rownr, REAL deltavalue)
{
  ...
  if(fabs(deltavalue) < lp->epsprimal) {
    /* Conversion to EQ */
    set_constr_type(lp, rownr, LPS_EQ);
  }
  else if(is_constr_type(lp, rownr, LPS_EQ)) {
    /* EQ with a non-zero range */
    if(deltavalue > 0)
      set_constr_type(lp, rownr, LPS_GE);
    else
      set_constr_type(lp, rownr, LPS_LE);
...
MYBOOL __WINAPI set_constr_type(lprec *lp, int rownr, int con_type)
{
  ...
  if(is_constr_type(lp, rownr, LPS_EQ))
    lp->equalities--;

  if((con_type & ROWTYPE_CONSTRAINT) == LPS_EQ) {
    lp->equalities++;
    lp->orig_upbo[rownr] = 0;
  }
  else if(((con_type & LPS_LE) > 0) || ((con_type & LPS_GE) > 0) || (con_type == LPS_FR))
    lp->orig_upbo[rownr] = lp->infinite;
  else {
...
{
  switch(con_type) {
    case LPS_FR: return("FR");
    case LPS_LE: return("LE");
    case LPS_GE: return("GE");
    case LPS_EQ: return("EQ");
    default: return("Error");
  }
}
...
*/
{
  ...
  if((lp->int_vars > 0) && (lp->solutionlimit == 1) && mat_validate(mat)) {
    ...
    if(n - intcount > 0) {
      ...
      ie = lp->rows;
      for(ib = 1; ib <= ie; ib++) {
        if(is_constr_type(lp, ib, LPS_EQ))
          break;
      }
      ...
      for(colnr = 1; colnr <= lp->columns; colnr++) {
        ...
        while(ib < ie) {
          if(is_constr_type(lp, (rownr = COL_MAT_ROWNR(ib)), LPS_EQ)) {
...
STATIC int findBasicFixedvar(lprec *lp, int afternr, MYBOOL slacksonly)
{
  ...
  for(; (afternr > 0) && (afternr <= lp->rows); afternr += delta) {
    ...
    if(((varnr <= lp->rows) && is_constr_type(lp, varnr, LPS_EQ)) ||
...
/* #define v51mode */ /* Enable this for v5.1 operation mode */
    ...
    if(((lp->simplex_mode & SIMPLEX_Phase1_DUAL) == 0) ||
       !is_constr_type(lp, rownr, LPS_EQ))                      /* *** DEBUG CODE KE */
#endif
...
/*    diff = my_reldiff(value, test); */
    ...
    if(diff > tolerance) {
      ...
      report(lp, errlevel,
        ...
        (is_constr_type(lp, i, LPS_EQ) ? "equality of" : "upper bound"), test);
      n++;
    }
...
/*    diff = my_reldiff(value, test); */
    ...
    if(diff < -tolerance) {
      ...
      report(lp, errlevel,
        ...
        get_row_name(lp, i), value,
        (is_constr_type(lp, i, LPS_EQ) ? "equality of" : "lower bound"), test);
      n++;
    }
  }
...
STATIC MYBOOL pre_MIPOBJ(lprec *lp)
{
  ...
  if(MIP_count(lp) > 0) {
    int i = 1;
    while((i <= lp->rows) && !mat_equalRows(lp->matA, 0, i) && !is_constr_type(lp, i, LPS_EQ))
      i++;
    if(i <= lp->rows)
      lp->constraintOF = i;
  }
