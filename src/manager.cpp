...
void Manager::bind_clock(sc_clock& clk, const std::string& name)
{
	m_clocks[name] = &clk;
}


void Manager::init_netlist()
{
	int addr = 1;
	int flow_id = 1;
...
void Manager::close_trace_file()
{
	sc_close_vcd_trace_file(m_trace_file);
}


sc_trace_file* Manager::get_trace_file()
{
	return m_trace_file;
}
